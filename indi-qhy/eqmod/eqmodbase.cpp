...
bool EQMod::updateProperties()
{
    ...
    if (isConnected())
    {
        ...
        try
        {
            ...
            // Auto-start tracking after a successful connection when the mount is unparked.
            // KStars may connect without explicitly toggling TRACK state.
            if (!isParked() && TrackState != SCOPE_TRACKING)
            {
                try
                {
                    const double raRate = GetDefaultRATrackRate();
                    const double deRate = GetDefaultDETrackRate();
                    mount->StartRATracking(raRate);
                    mount->StartDETracking(deRate);
                    TrackState         = SCOPE_TRACKING;
                    RememberTrackState = TrackState;
                    LOGF_INFO("Auto tracking enabled on connect (RA=%g arcsec/s, DE=%g arcsec/s).", raRate, deRate);
                }
                catch (EQModError &e)
                {
                    LOGF_WARN("Failed to auto-enable tracking on connect: %s", e.message);
                }
            }
        }
        catch (EQModError &e)
...
void EQMod::TimerHit()
{
    if (isConnected())
    {
        ...
        // Faster polling while slewing keeps KStars coordinates responsive.
        int poll = getCurrentPollingPeriod();
        if (TrackState == SCOPE_SLEWING)
            poll = std::max(50, std::min(500, poll / 4));
        SetTimer(poll);
    }
}
...
bool EQMod::ReadScopeStatus()
{
    ...
    try
    {
        ...
        // QHY mount firmware returns astronomical RA/DEC directly.
        // Do not pass values through legacy encoder->RA/DEC conversion.
        ...
        // Keep stepper panel populated with live position values for QHY workflow.
        steppervalues[0] = currentRA;
        steppervalues[1] = currentDEC;
            ...
            if (!(mount->IsRARunning()) && !(mount->IsDERunning()))
            {
                         ...
                         ((3600 * fabs(gotoparams.detarget - currentDEC)) > DEGOTORESOLUTION)))
                {
                    ...
                    LOGF_INFO(
                        ...
                        gotoparams.iterative_count, gotoparams.ratarget, gotoparams.detarget,
                        gotoparams.racurrent, gotoparams.decurrent);
                ...
                else
                {
                    ...
                    const bool trackingSuppressedForGoto = suppressNextGotoTracking;
                    const bool startTrackingAfterGoto =
                        !trackingSuppressedForGoto &&
                        ((RememberTrackState == SCOPE_TRACKING) || ((sw != nullptr) && (sw->s == ISS_ON)));
                    suppressNextGotoTracking = false;

                    if (startTrackingAfterGoto)
                    {
                        char *name;
                    ...
                    else
                    {
                        ...
                        RememberTrackState = TrackState;
                        if (trackingSuppressedForGoto)
                            LOG_INFO("Telescope slew is complete. Tracking suppressed for this goto.");
                        else
        ...
        if (TrackState == SCOPE_PARKING)
        {
            if (!(mount->IsRARunning()) && !(mount->IsDERunning()))
            {
                ...
                if ((3600 * fabs(Park_RA - currentRA)) > RAGOTORESOLUTION ||
                    (3600 * fabs(Park_DEC - currentDEC)) > DEGOTORESOLUTION)
                {
...
bool EQMod::Goto(double r, double d)
{
    ...
    // QHY mount goto is RA/DEC direct in firmware path.
    gotoparams.outsidelimits = false;
...
bool EQMod::Sync(double ra, double dec)
{
    ...
    // QHY mount reports telescope RA/DEC directly.
...
bool EQMod::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
    ...
    if (strcmp(dev, getDeviceName()) == 0)
    {
        ...
        if (strcmp(name, "SLEWSPEEDS") == 0)
        {
            ...
            if (TrackState != SCOPE_TRACKING)
            {
                // QHY path: these are UI slew presets.
                // Do not push them to firmware immediately on connection/config load,
                // otherwise motor periods are overwritten before user motion commands.
            }
...
bool EQMod::Abort()
{
    ...
    if (pulseInProgress & 1)
    {
        pulseInProgress &= ~1;
        IERmTimer(GuideTimerNS);
        GuideComplete(AXIS_DE);
    }

    if (pulseInProgress & 2)
    {
        pulseInProgress &= ~2;
        IERmTimer(GuideTimerWE);
        GuideComplete(AXIS_RA);
    }
...
{
    // QHY path uses astronomical coordinates directly.
